...
export default function ExplorePage() {
  ...
  return (
    <main className="mx-auto min-h-screen max-w-md bg-background px-5 pb-28">
      {/* header */}
      <div className="pt-6">
        <h1 className="font-title text-3xl font-bold text-ink">Explorer</h1>
        <p className="mt-1 text-sm text-muted">Cherche un sujet brûlant ou plonge par catégorie.</p>
      </div>
      ...
      <div className="mt-4 flex items-center gap-2 rounded-2xl border border-line bg-white px-4 py-3">
        ...
        <input
          type="text"
          placeholder="Rechercher un topique, un thème…"
          className="w-full bg-transparent text-sm text-ink outline-none placeholder:text-faint"
        />
      </div>

      {/* en feu maintenant */}
      <h2 className="mt-6 text-sm font-semibold uppercase tracking-wide text-brand">— En feu maintenant</h2>
      ...
      {/* catégories */}
      <h2 className="mt-8 text-sm font-semibold uppercase tracking-wide text-brand">— Catégories</h2>
  ...
  return (
    <button className="flex items-center gap-3 rounded-2xl border border-line bg-white p-4 text-left">
      ...
      <div className="min-w-0">
        ...
        <p className="text-xs text-faint">{category.topics}</p>
      </div>
    </button>
  );
}
